...
<!--确认订单页样式-->
 ...
 <div class="Orders_style clearfix" >
  ...
  @foreach($address as $ads)
    <input type="hidden" value="{{$ads->id}}">
   <!--地址信息样式-->
   <div class="Address_info" style="margin-bottom: 5px;">
    <div class="title_name">默认收货地址<a href="/deladdress/{{$ads->id}}" style="margin-left: 5px">删除</a><a href="/orderaddress">添加收货地址</a></div>
    <ul>
     <li><label>收件人姓名：</label><span id="name">{{$ads->name}}</span></li>
     <li><label>收件人地址：</label><span id="address">{{$ads->address}}</span></li>
     <li><label>收件人电话：</label><span id="phone">{{$ads->phone}}</span></li>
     <li><label>邮&nbsp;&nbsp;&nbsp;&nbsp;编：</label><span id="pc">{{$ads->pc}}</span></li>
    </ul>
   </div>
  @endforeach
  @else
    <div style="font-size: 20px;border: 1px solid #ff7200;line-height: 50px;">
      该用户现在没有地址,<a href="/orderaddress" style="color: red;">添加收货地址</a>
    </div>
  @endif

 </div>

 <form class="form" method="post" action="/orderuser"> 
 ...
 <!--快递选择-->
     <div class="express_delivery">
       <div class="title_name">选择快递方式</div>  
        <ul class="dowebok">
      <li><input type="radio" name="radio" data-labelauty="包邮">
     ...
     <div class="product_List">
      <table>
       <thead><tr class="title"><td class="name">商品名称</td><td class="price">商品价格</td><td class="Preferential">库存</td><td class="Quantity">购买数量</td><td class="Money">金额</td></tr></thead>
       <tbody>
      @foreach($data as $v)
       <tr>
        <td class="Product_info">
         <a href="#"><img src="{{$v['pic']}}"  width="120px" height="100px"/></a>
         <a href="#" class="product_name" style="line-height: 123px;">{{$v['name']}}&nbsp;{{$v['descr']}}&nbsp;{{$v['color']}}</a>
         </td>
        <td><i>￥</i>{{$v['price']}}</td>
        <td>{{$v['store']}}</td>
        <td>{{$v['num']}}</td>
        <td class="Moneys"><i>￥</i>{{$v['tot']}}</td>
       </tr>
       @endforeach
       </tbody>
      </table>
...
$('.Address_info').click(function(){
    $(this).css('border','3px solid red').siblings('.Address_info').css('border','3px solid #f90');
    // 获取选中的地址id
    $id=$(this).parents('.Orders_style').find(':hidden').val();
    $('#addressid').val($id);
});
