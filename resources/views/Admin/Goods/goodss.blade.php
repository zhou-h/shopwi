...
<head>
        ...
        <table class="mws-table mws-datatable dataTable" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info"> 
         <thead> 
          <tr role="row">
           ...
           <th class="sorting_asc" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 213px;">推荐</th>
         ...
         @foreach($goods as $v)
          <tr class="odd"> 
           ...
           <td class=" ">{{$v->descr}}</td>
           <td class=" ">
            <select class="large" name="display" id="display" >
                @if($v->static=='显示')
           ...
           <td class=" ">
            <select class="large recom" name="recom" id="recom" >
                @if($v->recom=='推荐')
    ...
    <script>
        ...
        $('.recom').change(function(){
            ...
            // alert(val);
            if(val==2 || val==3){
            $.get('/goodsedits',{'id':id,'recom':val},function(data){
                // alert(data);
                if(data){
                  // alert('1111');
                    $.session.set('success', '修改成功!')
